...
export async function POST(req: Request) {
    ...
    const gamesArray = answer.split("Game ").filter(Boolean).map((gameString) => {
		const [name, roundsString] = gameString.split(":");
		const roundsArray = roundsString.trim().split(";").map((roundString) => {
		  const colorsCounts = roundString.trim().split(",").map((colorCount) => {
    ...
    const findValidGames = gamesArray.map((game) => {
        ...
        const validRounds = rounds.map((round) => {
			return Object.keys(round).every((color) => allowance[color] >= round[color]);
		})
		const validGame = Object.values(validRounds).every((round) => round === true);
